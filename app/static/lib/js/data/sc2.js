...
function generate_scatter_plot1(selected_zcta)
{
	...
	var selected_suit_type = $("#scatter_plot1_select_suit_type :selected").text();
...
function generate_scatter_plot2(selected_ind)
{
	// var selected_ind = $("#scatter_plot1_select_ind :selected").text();
	// var selected_occ = $("#scatter_plot1_select_occ2 :selected").text();
	var selected_suit_type = $("#scatter_plot1_select_suit_type :selected").text();
	// console.log(selected_ind + " " + selected_occ + " " + selected_suit_type);
	for(var i = 0; i < industry_list.length; i++)
	{
		if(selected_ind == industry_list[i])
		{
			ind_index = i;
			break;
		}
	...
	scatter_plot1_data = [];
	for (var i = 0; i < current_suit.length; i++) {
		scatter_plot1_data.push([current_suit[i][ind_index]]);
	}

	for (var i = 0; i < zcta_vs_ind_lq.length; i++) {
		scatter_plot1_data[i].push(zcta_vs_ind_lq[i][ind_index]);
	}

	for (var i = 0; i < scatter_plot1_data.length; i++) {
		scatter_plot1_data[i].key = i + 1;
		scatter_plot1_data[i].name = zcta_list[i];
		scatter_plot1_data[i].type = "zcta";
	}
